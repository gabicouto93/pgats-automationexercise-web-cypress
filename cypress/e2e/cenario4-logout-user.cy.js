...
describe('Cenario4 Logout User', () => {
  ...
  before(() => {
    // Arrange: criar um usuário válido via UI e sair (garantindo independência do cenário)
    cy.visit('/');
    common.navegarParaSignupLogin();
    cy.contains('h2', 'New User Signup!').should('be.visible');

    // Pré-cadastro (name/email) e avanço
    ...
    // Verificar login e sair para preparar o cenário
  ...
  it('Deve logar e realizar logout com sucesso', () => {
    // 3. Verificar home
    ...
    // 6-7. Preencher login com credenciais válidas
    ...
    // 10. Verificar que foi redirecionado para a página de login
    cy.url().should('include', '/login');
    cy.contains('h2', /Login to your account/i).should('be.visible');
  });
});